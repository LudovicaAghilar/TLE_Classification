...
from resnet_MM import resnet18   # Assicurati che resnet.py sia nel PYTHONPATH
import json

# --------------------- PARAMETRI ---------------------
DATA_DIR    = r"C:\Users\ludov\Scripts\registered_linear_l\cropped\hist_normed_2\R1\test"
OUT_DIR     = r"C:\Users\ludov\Scripts\registered_linear_l\cropped\hist_normed_2\multimodal\HeatMap_avg_vanilla_smooth"
TARGET_LAYER = "layer4"
...
TEST_TXT_PATH = r"C:\Users\ludov\Scripts\results\optuna_mm_resnet18\test_predictions_fold_0.txt"
...
class GuidedBackprop3D_Smooth:
    ...
    def generate(self, x, sign=+1):
        ...
        for _ in range(self.n_samples):
            ...
            grad = noisy.grad.detach()      # (1,4,D,H,W)
            grad = grad.mean(dim=1)         # (1,D,H,W)
...
for path in nii_files:
    ...
    try:
        ...
        # DATA_DIR = ...\hist_normed_2\R1\test
        DATA_ROOT = os.path.dirname(os.path.dirname(DATA_DIR))

        contrasts = ['R1', 'R2', 'PD', 'MT']
        volumes = []

        for c in contrasts:
            vol_path = os.path.join(DATA_ROOT, c, "test", f"{subj_prefix}.nii")
            if not os.path.exists(vol_path):
                raise FileNotFoundError(f"{vol_path} non trovato")

            vol_c = load_volume(vol_path)  # (1, D, H, W)
            volumes.append(vol_c)


        # Stack in canale dimensione
        vol = torch.cat(volumes, dim=0).unsqueeze(0).to(DEVICE)  # (1,4,D,H,W)

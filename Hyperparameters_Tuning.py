...
results_dir = os.path.join('results', f"optuna_singolo_{contrast}_resnet18_one_trail")
plots_dir = os.path.join('plots', f"optuna_singolo_{contrast}_resnet18_one_trail")
...
def objective(trial):
    ...
    print(f"\nStarting trial {trial.number}: lr={lr:.2e}, weight_decay={weight_decay:.2e}, dropout={dropout:.2e}, batch_size={batch_size}, device={device}")

    # --- Load dataset and prepare folds (paths hard-coded like in your main)
    excel_path = r'C:\Users\ludov\Scripts\dati_cnr_new.xlsx'
    base_dir = r'C:\Users\ludov\Scripts\registered_linear_l\cropped\hist_normed_2\R1'
    train_val_dir = os.path.join(base_dir, f'train')

    label_mapping = load_label_mapping(excel_path)
    all_files = [f for f in os.listdir(train_val_dir) if f.endswith(".nii")]
    train_val_subjects = [create_subject(os.path.join(train_val_dir, f), label_mapping[os.path.splitext(f)[0]], os.path.splitext(f)[0])
                          for f in all_files if os.path.splitext(f)[0] in label_mapping]
